...
RSpec.describe 'Update Cultivating Life Cycle Event Mutation', type: :graphql_mutation do
  let(:current_user) { build(:user, :readwrite) }
  let(:specimen) { create(:specimen, owned_by: current_user.email) }
  let(:life_cycle_event) { create(:cultivating_event, specimen: specimen) }
  let(:query_string) {
    <<-GRAPHQL
		mutation($input: UpdateCultivatingLifeCycleEventInput!){
			updateCultivatingEvent(input: $input){
        errors{
          field
          value
          message
  ...
  context 'with valid parameters' do
    before :each do
      @life_cycle_event_id = PlantApiSchema.id_from_object(life_cycle_event, CultivatingEvent, {})
    ...
    it 'updates a record' do
      success_result = @result['data']['updateCultivatingEvent']['cultivatingEvent']
