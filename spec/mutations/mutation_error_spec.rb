...
RSpec.describe 'Mutation Error' do
  context 'when the error is not generated by ActiveRecord' do
    ...
    let(:image) { create(:image, imageable: create(:plant, owned_by: current_user.email), owned_by: current_user.email) }
  ...
  context 'when the error is generated by ActiveRecord' do
    context 'when a record fails persistence' do
      let(:current_user) { build(:user) }
      let(:existing_record) { create(:image, imageable: create(:plant, owned_by: current_user.email), owned_by: current_user.email) }
