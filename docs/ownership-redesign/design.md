...
- Cross-org references: creating a variety/specimen under another org's parent requires only read access to the parent; grants no edit rights either direction. Public children must not leak hidden parents — parent field resolvers return null/tombstone when the parent is not visible to the requester (verified in tests). Parent soft-deletion neither cascades to nor is vetoed by cross-org children.
...
Cutover (flag): legacy email/trust-9 branches become log-only (denials logged, not enforced) → removed in cleanup with evidence. Server-computed capability fields (`canEdit`, `canDelete`, `canRestore`, `canTransferOwnership`, on all six types) are contract-tested to agree with Pundit, so clients stop reproducing policy.
