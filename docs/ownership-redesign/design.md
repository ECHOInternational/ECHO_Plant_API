...
Cutover (flag): `ORG_AUTHZ_CUTOVER=log_only` made `OwnedResourcePolicy#log_legacy_divergence` emit an `authz.legacy_divergence` event whenever access was granted only by a legacy branch. **S7 shipped on 2026-08-04**: the legacy branches, the divergence logging and the flag itself (including its Terraform variable) are all gone, so the union below now reads as the organization rules alone, with trust >= 10 as the single global bypass. Server-computed capability fields (`canEdit`, `canDelete`, `canRestore`, on all six types) are contract-tested to agree with Pundit, so clients stop reproducing policy.
