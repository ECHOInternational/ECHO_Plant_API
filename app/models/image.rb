class Image < ApplicationRecord
	...
	# delegate :owned_by, to: :imageable

	# Raise an error when trying to update readonly fields
	def s3_key=(val)
	  raise ActiveRecord::ReadOnlyRecord.new("s3_key is readonly") if persisted?
	  super
	end

	def s3_bucket=(val)
	  raise ActiveRecord::ReadOnlyRecord.new("s3_bucket is readonly") if persisted?
	  super
	end
