...
class Principal < ApplicationRecord
  ...
  private_class_method def self.refresh_if_stale!(record, email:, display_name:)
    stale_auth = record.last_authenticated_at.nil? ||
                 record.last_authenticated_at < 1.hour.ago
    changed_fields = {}
    changed_fields[:email]        = email        if email.present? && record.email != email
    changed_fields[:display_name] = display_name if display_name.present? && record.display_name != display_name
    changed_fields[:last_authenticated_at] = Time.current if stale_auth
